...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Мар 19 2024 г., 16:57
...
INSERT INTO `History` (`id`, `time_start`, `city`, `fare`, `time`) VALUES
(1, '18:15:00', 1, 5, 5),
(2, '18:30:00', 2, 20, 7),
(3, '18:45:00', 1, 5, 15);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `phone_history`  AS SELECT `history`.`id` AS `ID`, `history`.`time_start` AS `Время начала`, `cities`.`city` AS `Город`, `history`.`fare` AS `Тариф`, `history`.`time` AS `Время разговора` FROM (`history` join `cities` on((`cities`.`id` = `history`.`city`)))  ;
